...
public abstract class AbstractNetworkTask extends AbstractTask {
  ...
  public AbstractNetworkTask(Logger logger, ResourceMonitor monitor) {
    ...
    try {
      ...
      addHost(Shared.EXAM_HOST);
      addHost(Shared.DIRECTORY_HOST);
      addHost(Shared.UPLOAD_HOST);
      addHost(Shared.VIDEO_HOST);
      addHost(Shared.LOG_HOST);
      addHost(Shared.CMS_HOST);
      addAllHosts();
  ...
  private String askLogService(String host) {
    ...
    WebTarget webTarget = client.target(Shared.BASE_LOG).path("lookup");

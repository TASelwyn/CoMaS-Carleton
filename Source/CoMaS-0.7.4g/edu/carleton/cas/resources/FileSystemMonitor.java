...
public class FileSystemMonitor extends AbstractResourceMonitor {
  ...
  public void open() {
    ...
    final File examDirectory = new File(this.baseDirectory, "exam");
    if (examDirectory.exists()) {
      this.examFiles = examDirectory.listFiles();
    } else {
      this.examFiles = null;
    } 
    final File resourcesDirectory = new File(this.baseDirectory, "resources");
    if (resourcesDirectory.exists()) {
      this.resourceFiles = resourcesDirectory.listFiles();
    } else {
                  ...
                  if (kind != StandardWatchEventKinds.OVERFLOW) {
                    ...
                    if (ev.kind() == StandardWatchEventKinds.ENTRY_DELETE) {
                      if (FileSystemMonitor.this.specialFile(filename, watchableString)) {
                        ...
                        FileSystemMonitor.this.logArchiver.put(Level.WARNING, String.valueOf(kind.toString()) + " " + 
                            filename.getFileName() + " from " + key.watchable());
                      } 
                    } else if (watchableString.equals(resourcesDirectory)) {
                      FileSystemMonitor.this.logArchiver.put(Level.WARNING, String.valueOf(kind.toString()) + " " + filename.getFileName() + 
                          " in " + key.watchable());
      ...
      TimerTask task = new TimerTask() {
          public void run() {
            if (FileSystemMonitor.this.exam_directory_events < Shared.MIN_EVENTS_IN_EXAM_DIRECTORY)
              FileSystemMonitor.this.notifyListeners("ALERT", "No activity in " + examDirectory + " for " + (

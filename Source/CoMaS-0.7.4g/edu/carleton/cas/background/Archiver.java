...
public abstract class Archiver implements ArchiverInterface {
              ...
              try {
                ...
                while (Archiver.this.isProcessing()) {
                  try {
                    ...
                    Logger.log(Level.WARNING, e.getMessage(), item);
                  } catch (Exception e) {
                    Logger.log(Level.SEVERE, e.getMessage(), item);
                  } 
                } 
              } finally {
  ...
  private void doTheWork(Object item) throws IOException, InterruptedException {
    ...
    if (!ok && this.statistics.getFailures() < Shared.MAX_FAILURES) {
      try {
        this.statistics.incrementFailures();
        Thread.sleep(this.login.randomInt(0, Shared.RETRY_TIME));
  ...
  public synchronized void stop() {
    if (!this.stopped.get()) {
      ...
      if (this.thread != null) {
        ...
        try {
          if (this.thread != null)
            this.thread.join(Shared.MAX_MSECS_TO_WAIT_TO_END); 
        } catch (InterruptedException interruptedException) {}
      } 
      this.working.set(false);
    } 
  ...
  public synchronized void put(Object item) {
    if (this.stopped.get() || this.statistics.getFailures() > Shared.MAX_FAILURES)
      return; 
    try {
      this.queue.put(item);
    } catch (InterruptedException interruptedException) {}

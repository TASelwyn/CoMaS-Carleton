              ...
              if (ExamDialog.this.isIdOkay(ExamDialog.this.studentID.getText()) && ExamDialog.this.isNameOkay(first, last)) {
                Thread loginProtocol = new Thread(new Runnable() {
                      public void run() {
                        ...
                        if (hasVMtechnology)
                          (ExamDialog.null.access$0(ExamDialog.null.this)).login.logArchiver.put((Level)Level.NOTED, "Virtual machine technology detected"); 
                        if (Shared.BLUETOOTH_MONITORING) {
                          Thread BTthread = new Thread(new Runnable() {
                                public void run() {
                                  ExamDialog.setStatus("0:BT check");
                                  if (LocalDevice.isPowerOn()) {
                                    (ExamDialog.null.access$0(ExamDialog.null.null.access$0(ExamDialog.null.null.this))).login.logArchiver.log((Level)Level.NOTED, "Bluetooth power on detected");

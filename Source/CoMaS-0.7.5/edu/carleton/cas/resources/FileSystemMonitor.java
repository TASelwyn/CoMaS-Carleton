...
public class FileSystemMonitor extends AbstractResourceMonitor {
  ...
  public FileSystemMonitor(LogArchiver logArchiver, String course, String activity) {
    ...
    this.comasDirectory = ClientShared.DIR;
    this.courseDirectory = ClientShared.getCourseDirectory(course);
    this.baseDirectory = ClientShared.getBaseDirectory(course, activity);
  ...
  public void open() {
    ...
    setupDirectoryMonitoring(this.desktopDirectory, ClientShared.DESKTOP_DIR);
                ...
                for (WatchEvent<?> event : key.pollEvents()) {
                  ...
                  if (kind != StandardWatchEventKinds.OVERFLOW) {
                    ...
                    if (ev.kind() == StandardWatchEventKinds.ENTRY_DELETE) {
                      ...
                      } else if (FileSystemMonitor.this.fileWeCareAbout(filename.getFileName())) {
    ...
    if (ClientShared.FREQUENCY_TO_CHECK_EXAM_DIRECTORY > 0) {
      this.watchServiceTimer = new Timer();
      TimerTask task = new TimerTask() {
          public void run() {
            if (FileSystemMonitor.this.exam_directory_events < ClientShared.MIN_EVENTS_IN_EXAM_DIRECTORY)
              FileSystemMonitor.this.notifyListeners("ALERT", "No activity in " + FileSystemMonitor.this.examDirectory + " for " + (
                  ClientShared.FREQUENCY_TO_CHECK_EXAM_DIRECTORY / 60000) + " mins"); 
            FileSystemMonitor.this.exam_directory_events = 0L;
          }
        };
      this.watchServiceTimer.scheduleAtFixedRate(task, ClientShared.FREQUENCY_TO_CHECK_EXAM_DIRECTORY, 
          ClientShared.FREQUENCY_TO_CHECK_EXAM_DIRECTORY);
    } 
  }
  ...
  private boolean fileWeCareAbout(Path name) {
  ...
  private boolean isSpecialDirectory(String file, String dir) {
    if (this.desktopDirectory.startsWith(dir) && file.equals(ClientShared.DESKTOP_DIR))
      return true; 

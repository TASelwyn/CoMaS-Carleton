  ...
  public ModuleContainer load(String name, String className, JarClassLoader classLoader) throws ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
    if (this.modules.contains(name)) {
      Logger.log(Level.WARNING, String.format(
            "Module named %s is already defined. It cannot be instantiated for %s", new Object[] { name, className }), "");
    ...
    if (clazz != null) {
      ...
      Logger.log(Level.WARNING, String.format("Unable to load %s using %s: ", new Object[] { name, className }), cnfe.getClass().getSimpleName());
      throw cnfe;
    } 
    return module;
  }
    ...
    if (this.actionInBackground) {
      ...
      try {
        ...
        Logger.log(Level.FINE, 
            String.format("%s module %s action exception: ", new Object[] { moduleContainer.getName(), action }), e);
      } 
    } 
  }
  ...
  public void configure(Properties properties) throws MalformedURLException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
    ...
    while (hiddenClassProp != null) {
      ...
      if (m.matches()) {
        ...
        Logger.log(Level.WARNING, 
            String.format("module.load.hidden.%d has an illegal class name format: %s", new Object[] { Integer.valueOf(i), hiddenClassProp }), "");
      } 
      i++;
    ...
    while (module != null) {
      ...
      if (definition != null && definition.length == 1) {
        mc = load(definition[0].trim(), definition[0].trim(), this.classLoaders[i - 1]);
      } else if (definition != null && definition.length == 2) {
        mc = load(definition[0].trim(), definition[1].trim(), this.classLoaders[i - 1]);
      } else {
        Logger.log(Level.WARNING, 
            String.format("module.load.%d has an illegal specification format: %s", new Object[] { Integer.valueOf(i), module }), "");
  ...
  public void send(String from, String to, String message) throws ModuleException {
    ...
    if (mc != null) {
      ...
      if (CommunicatingModule.class.isAssignableFrom(clazz)) {
        ...
        throw new ModuleException(String.valueOf(to) + " is not a communicating module", 
            new ClassCastException(clazz.getSimpleName()));
      } 
    } else {
  ...
  private class ModuleContainerProcessor implements Runnable {
    ...
    public void run() {
      try {
        ...
        Logger.log(Level.FINE, 
            String.format("%s module %s action exception: ", new Object[] { this.moduleContainer.getName(), this.action }), e);
      } 
    }
  }
}
